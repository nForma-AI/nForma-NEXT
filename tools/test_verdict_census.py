#!/usr/bin/env python3
"""Gated caller for `tools/verdict-census.py --self-test`. answers #2 by RUNNING every indexed instrument; --ledger keeps the EVER record and --stale-check asks in 0.085s whether it is current.
...
★ `verdict-census.py`'s controls lived behind `--self-test`, and the gate runs
...
SUBJECT = os.path.join(HERE, "verdict-census.py")
...
def main():
    ...
    for line in out.splitlines():
        print(f"  {line}")
    if r.returncode == 0:
        print("  ok    verdict-census --self-test: every reachable control passed")
        return 0
    if r.returncode == 3:
        print("  ⛔ verdict-census --self-test reported a FAILED control (exit 3)")
        return 1
    print(f"  VOID  verdict-census --self-test exited {r.returncode}, which it does not document"
