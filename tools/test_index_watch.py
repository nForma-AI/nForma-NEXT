#!/usr/bin/env python3
"""Gated caller for `tools/index-watch.py --self-test`. runs this role's cheap checkers when main moves, on two legs.
...
★ `index-watch.py`'s controls lived behind `--self-test`, and the gate runs
...
SUBJECT = os.path.join(HERE, "index-watch.py")
...
def main():
    ...
    for line in out.splitlines():
        print(f"  {line}")
    if r.returncode == 0:
        print("  ok    index-watch --self-test: every reachable control passed")
        return 0
    if r.returncode == 3:
        print("  ⛔ index-watch --self-test reported a FAILED control (exit 3)")
        return 1
    print(f"  VOID  index-watch --self-test exited {r.returncode}, which it does not document"
