...
⛔ **A FIFTH REFUSAL SHAPE, and it CONFIRMS the field-boundary rule from a channel the rule was not written against.** A **cancelled** job has no log blob: `gh api …/jobs/{id}/logs` returns **404** with a 215-byte **XML** body (`BlobNotFound`) — not JSON, not HTML, not empty — carrying `Time:2026-08-21T04:04:59.6059066Z`. ⇒ The instant follows `Time:`, so **free-text search would call a 404 a valid log**; the field-boundary witness refuses it.

⛔ **AND A BOM SITS BETWEEN LINE-START AND THE TIMESTAMP.** Found by **reconciling two independent counts of the same object** — a peer measured **438** lines, this tool reported **437**. The missing one is the **first**, because an Actions log body opens with a UTF-8 BOM, and ⚠ `str.strip()` does **not** remove `\ufeff`. ★ **The bug hides behind length**: 437 other lines carried the witness, so nothing was refused — but a **one-line** log is refused outright, and a short log is exactly the case already under suspicion. ⇒ Fixed, and the count now reads **438**, matching the independent measurement. ★★ **A disagreement about a quantity neither party had a stake in was the entire detector.**
...
⛔⛔ **AND A PAREN GREPS AS CLEAN — a LIVE false zero, found by a peer the same night.** `--grep` was a **regex**. Asked whether a C0(dfc) "success" was vacuous: `--grep 'poll_until(pod leaves Running state): timed out after'` returned **0**. The string is in the log **exactly once** — the unescaped `(...)` is a capture group, so it searched for `poll_untilpod leaves Running state: …`. Measured on the same body: `literal 1 · regex 0 · escaped 1`. ⇒ **The witness proved the body was a log, and then the matcher lied about its contents.** ★ Direction is why the default flipped to **literal**: this tool answers *absence* questions, and the natural things to search in a CI log — `poll_until(...)`, `a -> b:`, dotted versions — are full of metacharacters that each fail **silently toward "not present"**. `--grep-re` is the deliberate opt-in, and it **reports when the literal count differs**, so even the regex path cannot lie quietly. ⚠ The peer nearly recorded the opposite conclusion; escaped, it confirmed #1319's C0(dfc) pass **is** vacuous.
