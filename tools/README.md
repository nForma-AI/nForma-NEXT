...
**`api-budget.py`** — ⛔⛔ **it is NOT one pool, and that mattered within an hour of shipping the first version.** Measured with `core` fully drained for the **second consecutive hour** while every pane reported *"we are rate limited"*: `core` **0/5000 EXHAUSTED**, `graphql` **3508/5000 — 69% free**, `search` untouched. ⇒ **"Rate limited" named ONE bucket and was read as all of them**, while the emptier pool sat idle; the heavy consumers are REST (`gh api repos/…`, `gh pr view`), and a read expressible as a GraphQL query spends from a different allowance. ⛔ The GitHub quota is **a set of pools shared by every agent and every tool**, and nothing showed a pane its own share. Measured 2026-08-20 while the pool sat at **0/5000 with 42 minutes to reset** and every pane's `gh` call 403ing: **10,214 invocations across nine live transcripts in one session**, 4,553 from a single role, **3,352 of them bare `gh api`**. ⚠⚠ **One invocation is not one API call** — `--limit`/`--paginate` paginate, `run view --log` downloads an archive, and `gh api graphql` costs by node count — so the count is a **lower bound on spend and is never reported as the spend**; the tool flags which invocations used a multi-call flag and refuses to guess a multiplier. ★ **The cost lands on whoever asks NEXT**: a pane that made no calls all session gets the 403, which is why per-role attribution is the useful output. ⛔⛔ **And observed calls are `demand ∧ availability`.** While a bucket is exhausted every request is refused, so the count collapses toward zero and reads as restraint — measured: **hour 16 of one session shows ZERO invocations across nine live panes**, which was the hour REST was fully drained, not an hour anyone chose to be frugal. ⇒ `observed_is_demand()` **refuses a demand claim during a 403 window**. ★ **Three axes of one defect, and the tool now catches all three: wrong population across BUCKETS → across PROCESSES → across AVAILABILITY.** ⛔⛔ **And a meter DELTA is not a COST.** Nine panes spend against one pool, so a before/after bracket attributes **their** calls to whoever measured — a fleet-wide retraction on 2026-08-21 withdrew every cost figure quoted that day. **Three confounds, pointing different ways:** concurrency makes the delta **too high**, a **floored** meter reads 0 while spend continues, and a **reset** mid-bracket makes it negative. ⇒ `cost_range()` returns `(0, delta)` or **`None`** — a range or nothing, never a point — because real attribution needs a per-process counter and nothing has one. ★ Same defect as the bucket error one axis over: **wrong population across BUCKETS, then across PROCESSES.** ⛔ A failed `rate_limit` read is **`None`, never a full pool** — that endpoint is *exempt* from the quota it reports, so a failure there is network or auth and printing a number would blame the wrong thing.
