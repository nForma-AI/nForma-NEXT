#!/usr/bin/env python3
"""Gated caller for `tools/label-exists.py --self-test`. a queue query on a label that does not exist is silent, not empty.
...
★ `label-exists.py`'s controls lived behind `--self-test`, and the gate runs
...
SUBJECT = os.path.join(HERE, "label-exists.py")
...
def main():
    ...
    for line in out.splitlines():
        print(f"  {line}")
    if r.returncode == 0:
        print("  ok    label-exists --self-test: every reachable control passed")
        return 0
    if r.returncode == 3:
        print("  ⛔ label-exists --self-test reported a FAILED control (exit 3)")
        return 1
    print(f"  VOID  label-exists --self-test exited {r.returncode}, which it does not document"
