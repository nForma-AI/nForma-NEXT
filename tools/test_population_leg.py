#!/usr/bin/env python3
"""Gated caller for `tools/population-leg.py --self-test`. does each --self-test consult anything outside the repository, or the forge.
...
★ `population-leg.py`'s controls lived behind `--self-test`, and the gate runs
...
SUBJECT = os.path.join(HERE, "population-leg.py")
...
def main():
    ...
    for line in out.splitlines():
        print(f"  {line}")
    if r.returncode == 0:
        print("  ok    population-leg --self-test: every reachable control passed")
        return 0
    if r.returncode == 3:
        print("  ⛔ population-leg --self-test reported a FAILED control (exit 3)")
        return 1
    print(f"  VOID  population-leg --self-test exited {r.returncode}, which it does not document"
